...
### Finish setup of fn
```bash
# setup fn for mainnet, swap mainnet for testnet if testing
...
```bash
bash -x node_init_mainnet_version.sh
```

## Create & Manage Validator
See [this section](https://wiki.findora.org/docs/validators/staking-guide#node-operations) of the official guide on how to create your validator and the commands for validator management using `fn`.
