...
We'll make notes along the way on the files needed (tmp.gen.keypair and priv_validator_key.json) that you should provide. You can run through the whole install and then update the files, restart docker and reconnect `fn` to your proper files to see your validator begin signing with the proper keys.
