...
## Setup
Remember to read our [Before I Set a Validator Up]/companion/new/before) section and make a plan before starting here. You'll want to have a non-root user account (with sudo access to run root commands is ok!) created and the ~/harmony directory setup.
...
[Here's our suggestions](/companion/new/blskeys) on how to create BLS Keys for your server.
<Callout emoji="ℹ️">
...
## Updating a Node
You won't need to update a node at this point. However when you're ready to upgrade see our [Upgrade Script](/post/scripts/upgrade) section if you'd like to script the upgrade process.
## Checking a Node
There's a few commands you're going to use here regularly to check things out, however if you set up our [Stats Script](/post/scripts/stats) you can use that to check your status and use these sparingly as needed.
## Checking for BINGO
